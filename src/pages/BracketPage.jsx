...
import { getTeamById, ROUND_OF_16_BRACKET, QUARTERFINALS_BRACKET, SEMIFINALS_BRACKET, THIRD_PLACE_BRACKET, FINAL_BRACKET, MATCH_LABELS } from '../lib/teams'
...
export default function BracketPage({ viewUserId = null }) {
  ...
  // Tercer puesto: los perdedores de las semis
  // El perdedor de una semi es el equipo que NO fue elegido como ganador
  const getLoser = (matchId) => {
    const match = sfMatches.find(m => m.matchId === matchId)
    if (!match || !match.homeTeam || !match.awayTeam) return null
    const winnerId = picks[matchId]
    if (!winnerId) return null
    return winnerId === match.homeTeam?.id ? match.awayTeam : match.homeTeam
  }

  const thirdPlaceMatches = THIRD_PLACE_BRACKET.map(m => ({
    matchId: m.matchId,
    homeTeam: getLoser(m.loserFrom),
    awayTeam: getLoser(m.loserFrom2),
  }))

  const finalMatch = FINAL_BRACKET.map(buildAdvancedMatch)

  const champion = picks['M104'] ? getTeamById(picks['M104']) : null
          ...
          <div className="card" style={{ marginBottom: '1.5rem' }}>
            ...
            <div className="card-body" style={{ padding: '1rem', overflowX: 'auto' }}>
              <div className="bracket-rounds">
                ...
                {/* 3er puesto */}
                <BracketRound title="3er puesto" matches={thirdPlaceMatches} picks={picks} onPick={handlePick} locked={locked} results={results} />
